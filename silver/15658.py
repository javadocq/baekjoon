...
# 2 초	512 MB	10644	5577	4609	53.001%
# 문제
...
def solution(numbers, op):
    max_val = float(-10**7)
    min_val = float('INF')
    
    add, minus, multiply, mul = op[0], op[1], op[2], op[3]

    def dfs(idx, current_res, add, sub, multi, mul):
        ...
        if add > 0:
            dfs(idx + 1, current_res + numbers[idx], add - 1, sub, multi, mul)
        if sub > 0:
            dfs(idx + 1, current_res - numbers[idx], add, sub - 1, multi, mul)
        if multi > 0:
            dfs(idx + 1, current_res * numbers[idx], add, sub, multi - 1, mul)
        if mul > 0:
            if current_res < 0:
                dfs(idx + 1, -(-current_res // numbers[idx]), add, sub, multi, mul - 1)
            else:
                dfs(idx + 1, current_res // numbers[idx], add, sub, multi, mul - 1)

    dfs(1, numbers[0], add, minus, multiply, mul)
