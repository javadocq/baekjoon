...
# 1 초 (추가 시간 없음)	1024 MB	2823	1157	960	42.972%
# 문제
...
def dfs(idx, street_snow, m, size):
    global max_size
    ...
    if idx + 1 < len(street_snow):
        dfs(idx + 1, street_snow, m-1, size + street_snow[idx+1])
    if idx + 2 < len(street_snow):
        dfs(idx + 2, street_snow, m-1, (size//2) + street_snow[idx+2])

def solution(m, street_snow):
    dfs(-1, street_snow, m, 1)

    print(max_size)

n, m = map(int, input().split())
street_snow = list(map(int, input().split()))

solution(m, street_snow)