...
# 1 초	128 MB	12122	5753	4002	46.900%
# 문제
...
def solution(win_number):
    ...
    for part, time in win_number:
        ...
        if last_Part == 1:
            if A_Goals < B_Goals:
                last_Part = 2
                A_Times += change_time(time) - last_Time
                last_Time = change_time(time)
            ...
            if A_Goals > B_Goals:
                last_Part = 1
                B_Times += change_time(time) - last_Time
                last_Time = change_time(time)
            elif A_Goals == B_Goals:
                last_Part = 0
                B_Times += change_time(time) - last_Time
    ...
    if last_Time != change_time("48:00"):
        if last_Part == 1:
            A_Times += (change_time("48:00") - last_Time)
        elif last_Part == 2:
            B_Times += (change_time("48:00") - last_Time)

    print(f"{A_Times // 60:02}:{A_Times % 60:02}")
    print(f"{B_Times // 60:02}:{B_Times % 60:02}")

n = int(input())
win_number = []
for _ in range(n):
    part, time = input().split()
    win_number.append([int(part), time])
