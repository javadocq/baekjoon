...
# 1 초 (추가 시간 없음)	1024 MB	87544	24073	17869	25.099%
# 문제
...
def solution(n,m,b):
    ...
    cnt_block_list = Counter(block_list)
    ...
    for target in range(257):
        ...
        for height, count in cnt_block_list.items():
            
            if height > target:
                remove_cnt += (height - target) * count
            else:
                add_cnt += (target - height) * count
        

        if remove_cnt + b >= add_cnt:
            time = remove_cnt * 2 + add_cnt
